'''
Utils for load data
construct multi negNum train data
'''
import numpy as np
...
class LoadData(object):
    def __init__(self, path, dataset):
        ...
        self.createTrainFile(path, dataset)
        self.Train_data, self.Test_data = self.construct_data()
    
    def createTrainFile(self, path, dataset):
        fw = open(path + dataset + '/trainNew_10.csv', 'w')
        fr = open(path + dataset + '/train.csv')
        line = fr.readline()
        numNeg = 10
        while line:
            fw.write(line.strip()+',1'+'\n')
            
            user_features = line.strip().split(',')[0]
            user_id = self.binded_users[user_features]
                       
            all_items = self.binded_items.values()
            
            for i in range(numNeg):             
                neg = np.random.randint(len(all_items))
                while neg in self.user_positive_list[user_id]:
                    neg = np.random.randint(len(all_items))
                neg_feature = self.item_map[neg]
                fw.write(user_features.strip()+','+neg_feature+',-1'+'\n')
            line = fr.readline()
        fw.close()
        fr.close()
    ...
    def bind_i(self, file):
        f = open(file)
        line = f.readline()
        i = len(self.binded_items)# dic{feature: id}
        while line:
