...
class LoadData(object):
    def __init__(self, path, dataset):
        ...
        self.trainfile = self.path + "trainNew.csv"
    ...
    def construct_data(self):
        X_user, X_item, Y = self.read_dataTrain(self.trainfile)
        Train_data = self.construct_dataset_train(X_user, X_item, Y )
        print("# of training:", len(X_user))
        X_user, X_item = self.read_dataTest(self.testfile)
        Test_data = self.construct_dataset_test(X_user, X_item)
        print("# of test:", len(X_user))
        return Train_data, Test_data

    def read_dataTrain(self, file):
        # read a data file;
        f = open(file)
        X_user = []
        X_item = []
        ...
        while line:
            ...
            Y.append([float(features[2])])
            line = f.readline()
        f.close()
        return X_user, X_item, Y
    
    def read_dataTest(self, file):
        f = open(file)
        X_user = []
        X_item = []
        line = f.readline()
    ...
    def construct_dataset_train(self, X_user, X_item, Y):
        Data_Dic = {}
        indexs = range(len(X_user))
        Data_Dic['X_user'] = [X_user[i] for i in indexs]
        Data_Dic['X_item'] = [X_item[i] for i in indexs]
        Data_Dic['Y'] = [Y[i] for i in indexs]
        return Data_Dic
    
    def construct_dataset_test(self, X_user, X_item):
        Data_Dic = {}
        indexs = range(len(X_user))
        Data_Dic['X_user'] = [X_user[i] for i in indexs]
        Data_Dic['X_item'] = [X_item[i] for i in indexs]
